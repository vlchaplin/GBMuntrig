...
class PHA2Reader : public PHAReader {
	...
	~PHA2Reader() {
		...
		if ( fxbCntl->status ) this->errclose();
		else this->close();
	};
	...
	int getNSpectra() {
		...
		FxbReadStat * fxbCntl = this->getFxb();
		if ( fxbCntl->move2hdu( (char*)"SPECTRUM" ) ) return -1;
		
		return fxbCntl->n_rows;
	
	};
	
	int binSortedIntersection(long& rowMIN, long& rowMAX, double& searchTime, FxbReadStat& stat);
	int findRow( double time, long& row );
	
	int getTimes( double * tstart, double * tstop, long startrow, long numrows=1 );
	//int getEbounds( EdgeSet< float >& edges );
	
	int getSpectra( double * dataTable, double * exposure=NULL, long startrow=1, long numrows=1 );
	
	template<typename C,typename E,typename T>
	int getSpectra( PHA2<C,E,T>& set, long startrow=1, long numrows=0 )
	{
