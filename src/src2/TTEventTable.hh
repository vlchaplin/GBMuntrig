...
class TTEventTable {
    ...
    */
	...
	void CopyEvents(float * times, float * mags, t_type tshift=0.0, int chShift=0) {
		long i = 0;
		while ( i < this->size ) {
			...
			if ( this->edges != NULL ) {
				...
				mags[i] = (float)( (*this->edges)[1][ch+chShift] + (*this->edges)[0][ch+chShift] ) / 2.0;
			}
	...
	void getLightcurveRates( Xtype tstart, Xtype tend, Xtype tstep, Rtype ** rates, long& nbins, m_type chmin, m_type chmax, t_type tshifted=0.0 ) {
        ...
        nbins = floor( (tend - tstart) / tstep) + 1;
        
        if ( (tstart+tshifted) > (end-1)->time + 1.0 ) return;
        if ( (tend+tshifted) < ev->time - 1.0 ) return;
		
		while (ev != end && keepGoing) {
            ...
            if ( j >= nbins )
            {
                if ( j > nbins + 1000 ) keepGoing=0; //pad the break point in case of minor issues with event sorting 
                ev++;
                continue;
            }
            ...
            //assumes ev channels are 1-based
			rates[j][ev->chan-1] += 1.0 / tstep;
			ev++;
